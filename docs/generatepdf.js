...
export async function generarPDF(formData, cotizacion, autoDealer) {
    ...
    try {
        ...
        const tableWidth = doc.internal.pageSize.width - 2 * margen;
        const columnWidths = [0.75 * tableWidth, 0.25 * tableWidth];

        const addHeader = () => {
            doc.setTextColor(colorTextoPrincipal);
            doc.setFontSize(14);
            doc.text('Slip de Cotización Flotillas', margen, margen + 15);
            ...
            doc.text(`Cotización generada ${fechaCotizacion}`, margen, margen + 22);
            ...
            const posicionXSegundoLogo = doc.internal.pageSize.width - margen - logoWidthLQUE;
            const posicionXPrimerLogo = posicionXSegundoLogo - logoWidthLGFE - espacioEntreLogos;
            
            doc.addImage('imgs/LGFE_DOC.png', 'PNG', posicionXPrimerLogo, margen, logoWidthLGFE, logoHeight);
            doc.addImage('imgs/LQUE_DOC.png', 'PNG', posicionXSegundoLogo, margen, logoWidthLQUE, logoHeight);
        };

        const addFooter = () => {
            doc.setFontSize(8);
            doc.setTextColor(colorTextoSecundario);
            doc.text('Este documento es una cotización y no constituye una póliza de seguro.', margen, doc.internal.pageSize.height - margen - 5);
        
            doc.setFontSize(10);
            doc.text(`Página ${doc.internal.getCurrentPageInfo().pageNumber} de ${doc.internal.getNumberOfPages()}`, doc.internal.pageSize.width - margen - 30, doc.internal.pageSize.height - margen - 5);
        };
        
        let y = margen + 35;
        
        const addSection = (title) => {
            if (y > doc.internal.pageSize.height - margen - 30) {
                addPageBreak();
            }
            doc.setTextColor(colorTextoPrincipal);
            doc.setFontSize(12);
            const lines = doc.splitTextToSize(title, doc.internal.pageSize.width - 2 * margen);
            lines.forEach(line => {
        ...
        const addSectionBold = (title, espacioAntes = false, espacioDespues = false) => {
            if (espacioAntes) {
                y += 8;
            }
            if (y > doc.internal.pageSize.height - margen - 30) {
        ...
        const addParagraph = (text, espacioAntes = false, espacioDespues = false) => {
            if (y > doc.internal.pageSize.height - margen - 30) {
                addPageBreak();
            }
            if (espacioAntes) {
            ...
            doc.setFontSize(10);
            doc.setTextColor(colorTextoPrincipal);
            const lines = doc.splitTextToSize(text, doc.internal.pageSize.width - 2 * margen);
            lines.forEach(line => {
                if (y > doc.internal.pageSize.height - margen - 30) {
                    addPageBreak();
                }
                doc.text(line, margen, y);
                y += doc.getTextDimensions('A').h * 1.5;
            });
            if (espacioDespues) {
                y += 8;
            }
        ...
        function addParagraphList(items, espacioAntes = false, espacioDespues = false) {
            if (espacioAntes) {
                y += 5;
            }
            doc.setFontSize(10);
            ...
            items.forEach((item, index) => {
                ...
                lines.forEach((line, lineIndex) => {
                    ...
                    const offset = lineIndex === 0 ? 10 : 20;
                    doc.text(line, margen + offset, y);
                    y += doc.getTextDimensions('A').h * 1.5;
                });
            });
            if (espacioDespues) {
        ...
        function addParagraphNumbers(items, espacioAntes = false, espacioDespues = false) {
            if (espacioAntes) {
                y += 5;
            }
            doc.setFontSize(10);
            ...
            items.forEach((item, index) => {
                ...
                const lines = doc.splitTextToSize(text, doc.internal.pageSize.width - 2 * margen - 10);
                lines.forEach((line, lineIndex) => {
                    if (y > doc.internal.pageSize.height - margen - 30) {
                        addPageBreak();
                    }
        ...
        const addRow = (label, value) => {
            ...
            doc.setTextColor(colorTextoSecundario);
            doc.text(value, margen + 70, y);
            y += 6;
        };
        
        const addTwoColumnTableRow = (text1, text2, isHeader = false) => {
            const cellPadding = 2;
            const textHeight1 = doc.getTextDimensions(text1, { maxWidth: columnWidths[0] - 2 * cellPadding }).h;
            const textHeight2 = doc.getTextDimensions(text2, { maxWidth: columnWidths[1] - 2 * cellPadding }).h;
            const lineHeight = Math.max(textHeight1, textHeight2) + 2 * cellPadding;
        
            if (y + lineHeight > doc.internal.pageSize.height - margen - 30) {
                addPageBreak();
            }
        
            doc.setFillColor(isHeader ? '#f3f4f6' : '#ffffff');
            doc.rect(margen, y, columnWidths[0], lineHeight, 'FD');
            doc.rect(margen + columnWidths[0], y, columnWidths[1], lineHeight, 'FD');
        
            doc.setTextColor(colorTextoPrincipal);
            doc.setFontSize(isHeader ? 11 : 10);
            doc.text(text1.trim(), margen + cellPadding, y + cellPadding + doc.getTextDimensions(text1.trim()).h, { maxWidth: columnWidths[0] - 2 * cellPadding });
            doc.text(text2.trim(), margen + columnWidths[0] + cellPadding, y + cellPadding + doc.getTextDimensions(text2.trim()).h, { maxWidth: columnWidths[1] - 2 * cellPadding });
        ...
        const addTwoColumnTableRowInverse = (text1, text2, isHeader = false) => {
            const cellPadding = 2;
            const columnWidthsInverse = [
                doc.internal.pageSize.width * 0.25 - margen,
                doc.internal.pageSize.width * 0.75 - margen
            ];
            const textHeight1 = doc.getTextDimensions(text1, { maxWidth: columnWidthsInverse[0] - 2 * cellPadding }).h;
            const textHeight2 = doc.getTextDimensions(text2, { maxWidth: columnWidthsInverse[1] - 2 * cellPadding }).h;
            const lineHeight = Math.max(textHeight1, textHeight2) + 2 * cellPadding;
            ...
            doc.setFillColor(isHeader ? '#f3f4f6' : '#ffffff');
            doc.setDrawColor(255, 255, 255); // Cambiar el color de borde a blanco (invisible)
        
            doc.rect(margen, y, columnWidthsInverse[0], lineHeight, 'FD');
            doc.rect(margen + columnWidthsInverse[0], y, columnWidthsInverse[1], lineHeight, 'FD');
        
            doc.setTextColor(colorTextoPrincipal);
            doc.setFontSize(isHeader ? 11 : 10);
            doc.text(text1.trim(), margen + cellPadding, y + cellPadding + doc.getTextDimensions(text1.trim()).h, { maxWidth: columnWidthsInverse[0] - 2 * cellPadding });
            doc.text(text2.trim(), margen + columnWidthsInverse[0] + cellPadding, y + cellPadding + doc.getTextDimensions(text2.trim()).h, { maxWidth: columnWidthsInverse[1] - 2 * cellPadding });
        ...
        const addSingleColumnTableRow = (text, isHeader = false) => {
            const cellPadding = 2;
            const textHeight = doc.getTextDimensions(text, { maxWidth: tableWidth - 2 * cellPadding }).h;
            const lineHeight = textHeight + 2 * cellPadding;
            ...
            doc.setFillColor(isHeader ? '#f3f4f6' : '#ffffff');
            doc.rect(margen, y, tableWidth, lineHeight, 'FD');
        
            doc.setTextColor(colorTextoPrincipal);
            doc.setFontSize(isHeader ? 11 : 10);
            doc.text(text.trim(), margen + cellPadding, y + cellPadding + doc.getTextDimensions(text.trim()).h, { maxWidth: tableWidth - 2 * cellPadding });
        ...
        addParagraph('Vehículos livianos de uso particular.');
        ...
        addParagraph('(****) Con excepción del modelo Soluto de la marca Kia al que se aplica deducible de 20% del monto indemnizable.',true);

        addSingleColumnTableRow('Automóviles y Camionetas SUV de Origen Chino / Indio - Particular');
        ...
        addSectionBold('COBERTURAS ADICIONALES',true);
        addParagraphList([
        ...
        addSectionBold('SERVICIOS ADICIONALES',true);
        addParagraphList([
        ...
        addSectionBold('CONDICIONES ESPECIALES',true);
        addParagraphNumbers([
        ...
        addSectionBold('Por Modelo:',true);
        ...
        addSectionBold('Por Estado De Conservación:',true);
        addParagraphList([
        ...
        addSectionBold('Por Otras Características Especiales:',true);
        addParagraphList([
